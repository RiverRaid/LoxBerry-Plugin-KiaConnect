...
# Wie lange das Fahrzeug eingesteckt und nicht mehr ladend bei vollem Akku
# stehen darf, bevor FULLPARKED=1 gesendet wird.
FULL_PARKED_HOURS = 3
...
def load_state() -> dict:
    if not os.path.exists(STATE_PATH):
        return {"last_full_charge_at": None, "full_since": None}
...
def update_battery_health_state(state: dict, soc: int, plugged, charging: int, now: datetime.datetime) -> tuple[int, int]:
    """Aktualisiert den Zustand fuer die beiden Batteriepflege-Hinweise und
    gibt (full_parked, recharge_needed) als 0/1 zurueck."""
    ...
    return full_parked, recharge_needed
...
def main() -> None:
    ...
    for vehicle_id in manager.vehicles:
        ...
        full_parked, recharge_needed = update_battery_health_state(state, soc, plugged, charging, now)

        print(
            f"  {vehicle.name}: SOC={soc}% RANGE={range_km}km "
            f"CHARGING={charging} PLUGGED={plugged} "
            f"FULLPARKED={full_parked} RECHARGE100={recharge_needed} "
