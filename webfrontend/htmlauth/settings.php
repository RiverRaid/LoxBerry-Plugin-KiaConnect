...
if (($_SERVER["REQUEST_METHOD"] ?? "") === "POST") {
	...
	if ($action === "add_vehicle") {
		if (count($vehicles) >= KIA2LOX_MAX_VEHICLES) {
			...
			header("Location: settings.php?vehicle=" . urlencode($id));
			exit;
		}
		...
		$stale_auto_refresh_enabled = !empty($_POST["stale_auto_refresh_enabled"]);
		...
		foreach ($vehicles as &$v) {
			if ($v["id"] === $id) {
				...
				$v["stale_auto_refresh_enabled"] = $stale_auto_refresh_enabled;
			}
		}
		unset($v);
		kia2lox_save_vehicles($vehicles);
...
?>
	...
	<div class="kia2lox-card<?php echo !empty($active["kia_connected"]) ? " kia2lox-card-connected" : ""; ?>" id="kia2lox-cred-card"
		...
		<form method="post" action="settings.php" autocomplete="off" id="kia2lox-cred-form">
	...
	<div class="kia2lox-card">
		...
		<?php else: ?>
			<form method="post" action="settings.php" id="kia2lox-ms-form">
	...
	<div class="kia2lox-card">
		...
		<form method="post" action="settings.php" id="kia2lox-interval-form">
	...
	<div class="kia2lox-card" id="kia2lox-warnings-card">
		...
		$stale_auto_refresh_enabled = $active["stale_auto_refresh_enabled"] ?? true;
		?>

		<form method="post" action="settings.php" id="kia2lox-warnings-form">
			...
			<div class="kia2lox-field">
				<label class="kia2lox-checkbox">
					<input type="checkbox" id="stale_auto_refresh_enabled" name="stale_auto_refresh_enabled" value="1" data-role="none"
					       <?php echo $stale_auto_refresh_enabled ? "checked" : ""; ?>>
					<?php echo htmlspecialchars(kia2lox_t("SETTINGS.LABEL_STALE_AUTO_REFRESH")); ?>
				</label>
				<p class="kia2lox-hint"><?php echo htmlspecialchars(kia2lox_t("SETTINGS.HINT_STALE_AUTO_REFRESH")); ?></p>
			</div>
	...
	?>
	<div class="kia2lox-card" id="kia2lox-http-card">
		<button type="button" class="kia2lox-card-head kia2lox-collapse-toggle" data-role="none" id="kia2lox-http-toggle" aria-expanded="false" aria-controls="kia2lox-http-body">
			<svg class="kia2lox-collapse-chevron" id="kia2lox-http-chevron" viewBox="0 0 24 24" width="18" height="18" aria-hidden="true"><path fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" d="M6 9l6 6 6-6"/></svg>
			<h2><?php echo htmlspecialchars(kia2lox_t("SETTINGS.HTTP_TITLE")); ?></h2>
		</button>

		<div id="kia2lox-http-body" class="kia2lox-collapse-body" hidden>
...
<script>
	...
	function kia2loxInitSaveGroup(name, fieldIds, btnId, extraCheck) {
		...
		var initial = fields.map(kia2loxFieldValue);

		function check() {
			var dirty = fields.some(function (el, i) { return kia2loxFieldValue(el) !== initial[i]; });
		...
		var group = {
			resetBaseline: function () {
				initial = fields.map(kia2loxFieldValue);
				check();
			}
		};
	...
	kia2loxInitSaveGroup(
		"warnings",
		["full_soc_threshold", "full_hours", "full_parked_hours", "recharge_reminder_days", "low_soc_threshold", "low_battery_hours", "stale_auto_refresh_enabled"],
	...
	// Einklappbare Boxen ("Warnungen", "HTTP-Befehle"): Zustand (auf/zu)
	// je Browser in localStorage merken, Standard ist eingeklappt. Ein
	// <button data-role="none"> statt eines <form> als Klickflaeche,
	// damit jQuery Mobile hier nichts umstrukturiert (bekanntes Problem
	// in diesem Projekt).
	function kia2loxInitCollapse(toggleId, bodyId, storageKey) {
	...
	kia2loxInitCollapse("kia2lox-warnings-toggle", "kia2lox-warnings-body", "kia2lox_warnings_collapsed");
	kia2loxInitCollapse("kia2lox-http-toggle", "kia2lox-http-body", "kia2lox_http_collapsed");
	...
	(function () {
		...
		function passiveTimesToday() {
			...
			// Festes Raster ab Fensterbeginn (oder Mitternacht ohne Fenster),
			// alle interval Minuten - entspricht should_poll_passive_now() in
			// bin/kia2lox_poll.py: die geplanten Zeiten haengen nur von
			// Intervall/Zeitfenster ab, nicht davon, wann zuletzt (auch
			// manuell/per HTTP-Trigger) abgefragt wurde.
		...
		// Ergaenzt eine Liste vorhergesagter Zeiten um die tatsaechlichen
		// heutigen Abrufe dieser Art aus POLL_LOG (auch fehlgeschlagene, und
		// auch den ausserplanmaessigen Force-Refresh bei veralteten Daten -
		// beides automatisches Plugin-Verhalten) - sonst wuerden vergangene
		// Abfragen aus der Liste verschwinden, sobald die Vorhersage selbst
		// erst ab jetzt/dem letzten Abruf zu zaehlen beginnt (siehe
		// passiveTimesToday()). Manuelle Aufrufe (source "manual" - "Jetzt
		// aktualisieren"-Button, HTTP-Trigger, oder gar kein "source"-Feld
		// bei aelteren Eintraegen aus einer Version vor dieser Unterscheidung)
		// werden bewusst ausgeblendet, da sie nicht Teil des eigentlichen
		// Zeitplans sind.
		function withLoggedTimes(kind, predictedTimes) {
			var loggedTimes = POLL_LOG.filter(function (e) { return e.kind === kind && e.source === "cron"; }).map(function (e) { return e.time; });
			return predictedTimes.concat(loggedTimes)
				.filter(function (v, i, arr) { return arr.indexOf(v) === i; })
				.sort();
		}

		function scheduleAll() {
			var passiveTimes = withLoggedTimes("passive", passiveTimesToday());
			passiveScheduleRow.style.display = passiveModeSelect.value === "never" ? "none" : "";
			renderPills(passiveSchedulePills, "passive", passiveTimes);

			var forceTimes = withLoggedTimes("force", Array.prototype.map.call(
